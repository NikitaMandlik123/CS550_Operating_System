...
static int __init proList_init(void){
   printk(KERN_INFO "The ProcessList LKM is initializing\n");   
   majorNumber = register_chrdev(0, DEVICE_NAME, &fops);
   if (majorNumber<0){
      printk(KERN_ALERT "ProcessList unable to register a major number\n");
      return majorNumber;
   }
   printk(KERN_INFO "Registered with the major number %d\n", majorNumber);

   proListClass = class_create(THIS_MODULE, CLASS_NAME);
   if (IS_ERR(proListClass)){
      unregister_chrdev(majorNumber, DEVICE_NAME);
      printk(KERN_ALERT "Device class not registered\n");
   ...
   printk(KERN_INFO "Succesfully created device class\n");
   return 0;
}
